"""
Step #7: Prune the network from Step 6
...
# Add the weights to the nodes. The weight will be the unit catchment's area.
# Below, we will remove nodes that are below a weight threshold
...
sql = f"""
DROP TABLE IF EXISTS merit_basins7_{id};

CREATE TABLE merit_basins7_{id} AS 
SELECT 
...
FROM merit_basins6_{id} as M
JOIN a_temp AS T ON M.comid3 = T.comid3
GROUP BY T.mergeid;
"""
