...
NEW APPROACH using NetworkX... experiment!
...
sql = f"""
SELECT comid2, unitarea, nextdown
...
df.set_index('comid2', inplace=True)
...
pruned_G = calculate_shreve_stream_order(pruned_G)
...
if DRAW:
    ...
    mylabels = {node: pruned_G.nodes[node]['shreve_order'] for node in pruned_G.nodes}
...
for node in pruned_G.nodes:
    ...
    sorder = pruned_G.nodes[node]['shreve_order']
...
JOIN a_temp AS T ON M.comid2 = T.comid2
GROUP BY T.mergeid;
"""
